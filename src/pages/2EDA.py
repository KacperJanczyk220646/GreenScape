...
def preprocess(df):
    ...
    df = df.groupby(by=["date", "Year", "months", "Neighborhood", "regions"]).mean()
...
def histogram(df, y):
    ...
    fig = px.histogram(
        ...
        nbins=30,
        text_auto=".2f",
        title="Histogram on Mothly level",
    )
    fig.add_hline(
        y=df[y].mean(),
        line_dash="dot",
...
def pie_charts(df, regions):
    ...
    fig.add_trace(
        go.Pie(
            labels=df_sorted["regions"],
            values=df_sorted["green_score"],
            name="Green Score",
        ),
        1,
        1,
    ...
    fig.add_trace(
        go.Pie(labels=df_sorted["regions"], values=df_sorted["income"], name="Income"),
        1,
        2,
    )
    fig.add_trace(
        go.Pie(
            labels=df_sorted["regions"],
            values=df_sorted["livability_score_x"],
            name="Livability",
        ),
        2,
        1,
    ...
    fig.add_trace(
        go.Pie(
            labels=df_sorted["regions"],
            values=df_sorted["Registered nuisance (number)"],
...
scatterplot_income = scatterplot(
    df_sorted,
    "income",
    "green_score",
    "Correlation between Income score and Green Score Index",
)
scatterplot_public_nuisance = scatterplot(
    df_sorted,
    "Registered nuisance (number)",
    "green_score",
...
df_groupby_neighborhoods = (
    df_sorted.groupby(by="Neighborhood")
    .agg(
        avg_green_score=("green_score", "mean"),
        avg_livability_score_x=("livability_score_x", "mean"),
        avg_total_houses=("TotalHouses", "mean"),
...
if selected_option == "Months":
    st.markdown(
        "**_The green score index is displayed on both charts on a monthly basis. The highest average green score for all months was recorded between September and October 2017 - 39.94 and the lowest was recorded in September 2015 - 16.72. As a result, the average green score for all the months was approximately 27._**"
    )
    with col1:
    ...
    st.markdown(
        "**_The green score index is displayed on both charts on a yearly basis. The highest average green score for all years was recorded on 2015 - 28.12 and the lowest was recorded on 2018 - 23.5._**"
    )
    with col1:
        st.plotly_chart(barplot_years, theme="streamlit", use_container_width=True)
    with col2:
        st.plotly_chart(linechart_years, theme="streamlit", use_container_width=True)
...
with tab3:
    st.markdown(
        "**_The scatter plot depicts the relationships between the livability score and the green score for all regions in Breda for the period of 2014 to 2018 according to the size of the dots indicating the population for all regions in Breda. As can be seen in the graph, the green score is indicated on the Y axis, while the livability is represented on the X axis._**"
    )
    st.plotly_chart(scatterplot_livability, theme="streamlit", use_container_width=True)
with tab4:
    st.markdown(
        "**_The scatter plot depicts the relationships between the income and the green score for all regions in Breda for the period of 2014 to 2018 according to the size of the dots indicating the population for all regions in Breda. As can be seen in the graph, the green score is indicated on the Y axis, while the income is represented on the X axis._**"
    )
    st.plotly_chart(scatterplot_income, theme="streamlit", use_container_width=True)
with tab5:
    st.markdown(
        "**_The scatter plot depicts the relationships between the registered public nuisance and the green score for all regions in Breda for the period of 2014 to 2018 according to the size of the dots indicating the population for all regions in Breda. As can be seen in the graph, the green score is indicated on the Y axis, while the registered public nuisance is represented on the X axis._**"
    )
    st.plotly_chart(
        scatterplot_public_nuisance, theme="streamlit", use_container_width=True
    )
...
with col3:
    ...
    st.markdown(
        "**_The heatmap is a graphical representation of data using colors to indicate the level of activity. Darker colors are used to indicate low activity and brighter colors to indicate high activity. Between green score and livability score there is moderate correlation as green score increases, the livability score decreases._**"
    )
with col4:
    dropdown_options1 = ["Years", "Months", "Regions", "Neighborhoods"]
    tables = {
        "Years": df_groupby_years["avg_green_score"],
